    ...
    private void updatePathWithWrapping(GeometryPath path, JSONArray bodyTransforms) {
        ...
        for (int ppointSetIndex=1; ppointSetIndex < pathPointSetNoWrap.getSize(); ppointSetIndex++) {
            ...
            if (secondIndex == firstIndex+1 || (firstIndex ==-1 && secondIndex!=-1)){
                ...
                // Tally number of actual points generated by wrapping API
                int totalNumWrapPoints = 0;
                ArrayList<PathWrapPoint> ptsToUse = new ArrayList<PathWrapPoint>();
                ...
                for (int wrappointIndex = firstIndex + 1; wrappointIndex < secondIndex; wrappointIndex++) {
                    ...
                    if (pathWrapPoint != null) {
                        ...
                        int sz = pathwrap.size();
                        if (sz >=1){
                            totalNumWrapPoints+=sz; 
                            ptsToUse.add(pathWrapPoint);
                            counts.add(sz);
                        }
                    }
                }
                ...
                for (int wpptindex = 0; wpptindex < counts.size(); wpptindex++) {
                    // Count how many wrap points in sequence and distribute 2 * numWrapObjects among them 
                    // for now we'll assume only one
                    PathWrapPoint nextWrapPoint = ptsToUse.get(wpptindex);
                    ArrayVec3 pathwrap = nextWrapPoint.getWrapPath();
                    PhysicalFrame wrapPtsFrame = nextWrapPoint.getParentFrame();
                    int numVisPoints = numIndicesToUse.get(wpptindex);
                    int numAPIPoints = counts.get(wpptindex);
                    ...
                    for (int ndx = 0; ndx < numVisPoints - 1; ndx++) {
                        indicesToUse[ndx] = ndx * (numAPIPoints - 1) / (numVisPoints);
                    }
                    // Force the last point to be the last point generated by API (work sround roundoff)
                    indicesToUse[numVisPoints - 1] = numAPIPoints - 1;
                    JSONObject bodyJson = mapBodyIndicesToJson.get(0); // These points live in Ground
                    ArrayList<UUID> wrapPointUUIDs = wrapPathPoints.get(nextWrapPoint);
                    if (wrapPointUUIDs == null) {
                        // Wrapping was never encountered, treat as new
                        if (verbose) {
                            System.out.println("New Contact Encountered muscle pt " + firstPoint.getName());
                        }
                        ...
                        for (int intermediatePpt = (1 + numIntermediatePoints) * firstIndex + 1;
                                intermediatePpt < (1 + numIntermediatePoints) * firstIndex + numVisPoints + 1;
                                intermediatePpt++) {
                            int test = intermediatePpt;
                            wrapPointUUIDs.add(UUID.fromString((String) pathpointJsonArray.get(intermediatePpt)));
                        }
                        wrapPathPoints.put(nextWrapPoint, wrapPointUUIDs);
                    }
