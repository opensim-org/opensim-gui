...
/**
 *  An arrow with start point, end point and tip-length. Note that <br>
 * the actual placement can be changed by the parent class transform &amp; scale; <br>
 * here we are just generating the initial arrow in the geometry object's local <br>
 * frame.<br>
 * <br>
 * There is a default constructor for this object but it is not much<br>
 * use unless followed by end point(s) specifications. By default <br>
 * we produce an arrow going from (0,0,0) to (1,1,1) and tip<br>
 * length of .35 just so it will show up if you forget to set it to something <br>
 * meaningful. Having a  default constructor allows us to have arrays of these <br>
 * objects. *
 */
