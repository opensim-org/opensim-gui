 ...
 * segment again.<br>
 * <br>
 * The path is computed as an optimization problem by computing natural geodesic<br>
 ...
 *     System Dynamics 36, 195–219.<br>
 * <br>
 * The overall path is locally the shortest, allowing winding over an obstacle<br>
 * multiple times, without flipping to the other side. By default, path<br>
 * solutions are computed from scratch using the user-specified "contact hints"<br>
 * associated with each wrap obstacle (see <a href="#appending_obstacles">appending_obstacles</a> for<br>
 * details). Alternatively, the wrapping solver can use the most recently<br>
 * computed solution (e.g., from a previous time step in a forward integration)<br>
 * as a warm start to improve performance (see method `getUseWarmStart()` for<br>
 * more information). In other scenarios where the configuration of the model,<br>
 * and therefore path solutions, may change rapidly, it is recommended to<br>
 * disable this warm starts to avoid inconsistent path solutions.<br>
 * <br>
...
public class Scholz2015GeometryPath extends AbstractGeometryPath {
  ...
  /**
   * Set whether the path uses the most recently computed solution as a warm<br>
   * start for the wrapping solver.<br>
   * <br>
   * Enable this setting when performing forward dynamics simulations or any<br>
   * simulation where path solutions will be computed sequentially in time<br>
   * (using reasonably small time steps). In other scenarios where the<br>
   * configuration of the model, and therefore path solutions, may change<br>
   * rapidly, it is recommended to disable this setting to avoid inconsistent<br>
   * path solutions. If false, the path will always be computed from the curve<br>
   * contact hints when realizing to SimTK::Stage::Position. Default: false.
   */
  public void setUseWarmStart(boolean useWarmStart) {
    opensimSimulationJNI.Scholz2015GeometryPath_setUseWarmStart(swigCPtr, this, useWarmStart);
  }

  /**
   * Get whether the path uses the most recently computed solution as a warm<br>
   * start for the wrapping solver.<br>
   * <br>
   * @see setUseWarmStart()
   */
  public boolean getUseWarmStart() {
    return opensimSimulationJNI.Scholz2015GeometryPath_getUseWarmStart(swigCPtr, this);
  }
