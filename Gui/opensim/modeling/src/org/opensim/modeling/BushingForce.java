 ...
 * change (e.g. Euler angle derivatives) which is NOT the angular velocity <br>
 * between the two frames. That makes this bushing model suitable only for <br>
 * relatively small relative orientation deviations between the frames.<br>
 * <br>
 * The underlying Force in Simbody is a SimtK::Force::LinearBushing. This <br>
 * implementation exposes the state variable for the dissipated energy of the <br>
 * bushing force allocated internally by Simbody.<br>
 * <br>
 * @author Ajay Seth
 */
public class BushingForce extends TwoFrameLinkerForce {
  ...
  /**
   * Obtain the total amount of energy dissipated by this BushingForce since <br>
   * some arbitrary starting point, in joules.<br>
   * <br>
   * This is the time integral of the power dissipation. For a system whose <br>
   * only non-conservative forces are Bushings, the sum of potential, kinetic, <br>
  ...
  /**
   * Set the accumulated dissipated energy to an arbitrary value, in joules.<br>
   * <br>
   * Typically this is used only to reset the dissipated energy to zero, but <br>
   * non-zero values can be useful if you are trying to match some existing <br>
  ...
  /**
   * Obtain the rate at which energy is being dissipated by this BushingForce, <br>
   ...
   * 'statebounds_dissipated_energy'. We don't need the state, but the state <br>
