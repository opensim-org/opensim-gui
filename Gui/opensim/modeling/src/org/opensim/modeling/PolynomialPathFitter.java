 ...
 * `setMinimumPolynomialOrder` and `setMaximumPolynomialOrder` methods specify<br>
 * the minimum and maximum order of the polynomial used to fit each path. The<br>
 * `setGlobalCoordinateSamplingBounds` property specifies the global bounds (in<br>
 * degrees) that determine the minimum and maximum coordinate values sampled at<br>
 * each time point. The method `appendCoordinateSamplingBounds` can be used to<br>
 * override the global bounds for a specific coordinate. The<br>
 * `setMomentArmTolerance` and `setPathLengthTolerance` methods specify the<br>
 * tolerance on the root-mean-square (RMS) error (in meters) between the moment<br>
 * arms and path lengths computed from the original model paths and the fitted<br>
 * polynomial paths. The `setNumSamplesPerFrame` method specifies the number of<br>
 * samples taken per time frame in the coordinate values table used to fit each<br>
 * path. The `setNumParallelThreads` method specifies the number of threads used<br>
 * to parallelize the path fitting process. The `setLatinHypercubeAlgorithm`<br>
 * method specifies the Latin hypercube sampling algorithm used to sample<br>
 * coordinate values for path fitting.<br>
 * <br>
...
public class PolynomialPathFitter extends OpenSimObject {
  ...
  /**
   * The directory relative to the working directory to which the path fitting<br>
  ...
  /**
   *  <b>(Deprecated)</b> Moment arms associated with a path are now<br>
   *  automatically detected based on the model's topology.
   */
  public void setMomentArmThreshold(double arg0) {
    opensimActuatorsAnalysesToolsJNI.PolynomialPathFitter_setMomentArmThreshold(swigCPtr, this, arg0);
  }
   ...
   * `Function` objects that approximate the original model paths.<br>
   * <br>
   * Note: Since this is a static helper function, the number of threads used<br>
   *       to parallelize the path length and moment arm computations is<br>
   *       determined by the number of available hardware threads, not via<br>
