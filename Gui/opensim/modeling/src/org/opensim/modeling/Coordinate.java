...
/**
 * A Coordinate is a ModelComponent for managing the access and behavior <br>
 * of a model's generalized coordinate including its value, speed and <br>
 * acceleration (once system accelerations have been realized). <br>
 * As a ModelComponent it provides resources to enable a Coordinate to be<br>
 * locked, prescribed, or clamped (limited to a min-to-max range).<br>
 * <br>
 * @author Ajay Seth, Ayman Habib, Michael Sherman 
 */
public class Coordinate extends ModelComponent {
  ...
  /**
   *  Specify an OpenSim Function specifies the prescribed motion for this <br>
   *         Coordinate as a function of time. Note, this function must provide<br>
  ...
  /**
   *  Motion type that describes the motion dictated by the coordinate.<br>
   *         Specifically it describes how the coordinate can be interpreted.<br>
   *         A coordinate can be interpreted as Rotational or Translational if<br>
   *         the displacement about or along an axis is the coordinate value.<br>
   *         If the Coordinate cannot be interpreted as being either of these<br>
